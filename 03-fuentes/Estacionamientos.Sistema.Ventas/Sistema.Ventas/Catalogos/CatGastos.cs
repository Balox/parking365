...
{
    public partial class CatGastos : Form
    {
        public CatGastos()
        {
            InitializeComponent();
        ...
        private void CargaGrid()
        {
            ...
            dt = new ClassGenerales().EjecutaQuery("SELECT iCveGasto as Codigo, cDesc as Categoria, bActivo as Activo FROM CatGastos;");
        ...
        private void CargaEdicion()
        {
            if (GridDatos.SelectedCells.Count > 0)
            {
                ...
                DataTable dt = new DataTable();
                dt = new ClassGenerales().EjecutaQuery("SELECT iCveGasto as Codigo, cDesc as Categoria, bActivo as Activo " +
                    " FROM CatGastos where  iCveGasto=" + iCveUsuarios);
        ...
        private void toolStripButton6_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    MessageBox.Show("Inserte un Gasto.", Variables.NombreEmpresa, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    txtCategoria.Focus();
                    return;
                }
                ...
                if (txtNo.Text == "Nuevo")
                {
                    ...
                    Inserto = new ClassGenerales().EjecutaQuery2("insert into CatGastos  (cDesc, bActivo) values " +
                ...
                else
                {
                    ...
                    Inserto = new ClassGenerales().EjecutaQuery2("update CatGastos set cDesc='" + txtCategoria.Text.Trim().ToUpper() +
                        "', bActivo=" + Status + " where iCveGasto=" + iCve);
                }

                if (Inserto == true)
                {
                    MessageBox.Show("Se Registro correctamente", Variables.NombreEmpresa, MessageBoxButtons.OK, MessageBoxIcon.Information);
                    iFormCargaCatalogoGastos formInterface = Application.OpenForms.OfType<Form>().Where(pre => pre.Name == "Gastos").SingleOrDefault() as iFormCargaCatalogoGastos;
                    if (formInterface != null)
                        formInterface.CargaGastos();
                    iFormCargaCatalogoGastos formInterface2 = Application.OpenForms.OfType<Form>().Where(pre => pre.Name == "GastosAuxiliar").SingleOrDefault() as iFormCargaCatalogoGastos;
                    if (formInterface2 != null)
                        formInterface2.CargaGastos();
                }
                else
                {
